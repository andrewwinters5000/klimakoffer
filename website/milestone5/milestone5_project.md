...
L_{\text{n\_lat},i} &= \frac{\sin(h/2)}{4 \pi \text{area[n\_lat]}}\sum^{\text{n\_longitude}}_{k=1}\frac{1}{2}\left(\tilde{D}_{\text{n\_lat},k}+\tilde{D}_{\text{n\_lat-1},k}\right)\left[T_{\text{n\_lat-1},k}-T_{\text{n\_lat},k}\right]
\label{polesDoF}
\end{align}
for all $i \in \{1,\dots,\text{n\_longitude}\}$, where $\texttt{area[1]} := \texttt{area[n\_lat]} := 0.5\left(1-\cos\left(h/2\right)\right)$ contains the normalized area of the polar cap. For formatting reasons n\_latitude is shortened to n\_lat here.
...
In code, we can do the same with the functions *flatten* and *reshape*, which are available in all languages for numerical computing.

This function requires many calls to *calc\_diffusion\_coefficients*.
...
To bring *calc\_diffusion\_coefficients* into the microsecond range, we recommend using Numba's *@njit* decorator.
